		...
		<div class="row">
			<div class="col-md-12 col-sm-12">
				...
				<nav aria-label="breadcrumb" role="navigation">
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="index.html">Listado de producción</a></li>
...
<div class="card-box mb-30">
	<div class="pd-20">
		<div class="row">
			<div class="col-sm-6">
				<h4 class="text-blue h4">Historial de las producciones registradas</h4>
			</div>
			<div class="col-sm-6">
				<a href="{{ route('pesaje/registrar') }}" class="btn btn-primary pull-right">+ Nueva producción</a>
			</div>
		</div>
		...
		<table class="data-table table stripe hover nowrap">
			<thead>
				<tr>
					...
					<th><center>Producción</center></th>
					<th><center>Fecha de inicio</center></th>
					<th><center>Fecha de cierre</center></th>
			...
			<tbody>
				@foreach ($producciones as $item)
					<tr>
						...
						<td><center>{{ $item->concepto->nombre }} </center></td>
						<td><center>{{ $item->valor_produccion }} {{ $item->unidad_medida->nombre }}</center></td>
						<td><center>{{ date("d/m/Y H:i", strtotime($item->fecha_inicio)) }}</center></td>
						<td><center>{{ date("d/m/Y H:i", strtotime($item->fecha_fin)) }}</center></td>
						<td title="{{  $item->observaciones }}">
							<center>
								{{ strlen($item->observaciones) <= 15 ? $item->observaciones : substr($item->observaciones, 0, 15).'...' }}
							</center>
						</td>
							...
							<div class="dropdown">
								...
								<div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
									<a class="dropdown-item" 
									   href="{{ route('produccion/editar') }}?produccion={{ $item->id_animal_produccion }}">
